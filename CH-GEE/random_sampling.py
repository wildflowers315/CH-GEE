...
def generate_sampling_sites(
    region: ee.Geometry, 
    cell_size: int, 
    seed: int, 
    mask_raster: ee.Image) -> Dict[str, Any]:
    # mask_raster: ee.Image) -> ee.FeatureCollection: #Dict[str, Any]:
    """
    Generate sampling sites using a grid-based approach.
...
def create_training_data(
    ...
    # Combine all data
    merged = s2_data.addBands(gedi_data).addBands(s1_data)
