...
if(Inputs.u_chooseAoi == "Draw"){
   ...
   if(Inputs.u_TypePerc1){
     if(Inputs.u_choose_mask1){
       var tree_heights = library.CanopyHeightMapper(Inputs.u_aoi, Inputs.u_year, Inputs.u_start_date,Inputs.u_end_date, Inputs.u_cloud_threshold, 
     ...
     Inputs.u_numTreesGBM,Inputs.u_shrGBM,Inputs.u_samLingRateGBM,Inputs.u_maxNodesGBM,Inputs.u_lossGBM,Inputs.u_maxNodesCART,Inputs.u_minLeafPopCART);
          ...
          tree_heights = library.CanopyHeightMapper(Inputs.u_aoi, Inputs.u_year, Inputs.u_start_date, 
     Inputs.u_end_date, Inputs.u_cloud_threshold, 'rh100', // Inputs.u_scale, // quantile by default
   ...
   // 
    ...
    if(Inputs.u_choose_mask1){
     ...
     Inputs.u_numTreesGBM,Inputs.u_shrGBM,Inputs.u_samLingRateGBM,Inputs.u_maxNodesGBM,Inputs.u_lossGBM,Inputs.u_maxNodesCART,Inputs.u_minLeafPopCART);
          ...
          tree_heights = library.CanopyHeightMapper(Inputs.u_aoi, Inputs.u_year, Inputs.u_start_date, 
     Inputs.u_end_date, Inputs.u_cloud_threshold, Inputs.u_quantile, // Inputs.u_scale, // quantile by default
  ...
  if(Inputs.u_DownloadOutput){
   ...
   if(Inputs.u_TypePerc1){
     if(Inputs.u_choose_mask1){
       var tree_heights_CC = library.CanopyHeightMapper(Inputs.u_aoi, Inputs.u_year, Inputs.u_start_date,Inputs.u_end_date, Inputs.u_cloud_threshold, 
       Inputs.u_quantile,'RF','DW','singleGEDI',Inputs.u_numTreesRF,Inputs.u_varSplitRF,Inputs.u_minLeafPopuRF,Inputs.u_bagFracRF,
       Inputs.u_maxNodesRF,Inputs.u_numTreesGBM,Inputs.u_shrGBM,Inputs.u_samLingRateGBM,Inputs.u_maxNodesGBM,Inputs.u_lossGBM,Inputs.u_maxNodesCART,
       Inputs.u_minLeafPopCART);
     }
...
backgroundColor : "#424457", color: "#598854", fontSize: "28px", fontFamily: "monospace"}});
//  ********************************* Subtitle  **********************************
...
var aoiShpTexbox = ui.Textbox({
  ...
  style: {shown: false, width: '335px',color: 'green',border: '1px solid darkgray'}
});
//  *********************************   AOI upload   *********************************  
var chooseAoiCheckSelector = ui.Select({
 items: [
   {label: 'Draw Area of Intereset (AOI) - Geometry tools'  , value: "Draw"    },
   {label: 'Upload Area of Interest (AOI) - Shapefile format '    , value: "Boundary"}],
style: {color: 'green',border: '2px solid darkgray',width: '335px',height: '30px',fontWeight: 'bold', fontSize: '20px'}
   }).setValue("Draw");
//  *********************************   Canopy heights map    ***************************
...
var YearTexbox = ui.Select({
  items: [
                  ...
                  style: {color: 'green',width: '100px',height: '30px', border: '2px solid darkgray'}
                  }).setValue("2023");
//  *********************************   Start date for Collecting images    *******************************
var startDateTexbox = ui.Textbox({
placeholder: 'startDate (e.g. 07-01)',
value: '07-01',
style: {width: '100px',height: '30px',color: 'green',border: '2px solid darkgray'}});
...
var u_outputImgNameTexbox = ui.Textbox({
 ...
 //  *********************************     Folder label maps      ****************************************
 var outputFolderLabel = ui.Label({value: "Drive Image Folder Name", style:{
...
var cloudThresholdLabel     =  ui.Label({value: "> Set cloud coverage percentage for Sentinel-2", 
...
//*********************************************************************************************************
var selectmask1 = ui.Checkbox({label: 'GOOGLE/DYNAMICWORLD/V1', value: false, style:{shown: true,
  backgroundColor : "#424457", color: "white", fontSize: "16px", fontFamily: "monospace"}});
var selectmask2 = ui.Checkbox({label: 'JAXA/ALOS/PALSAR/YEARLY/FNF', value: false, style:{shown: true,
  backgroundColor : "#424457", color: "white", fontSize: "16px", fontFamily: "monospace"}});
  var selectmask3 = ui.Checkbox({label: 'Non-ForestMask', value: false, style:{shown: true,
  backgroundColor : "#424457", color: "white", fontSize: "16px", fontFamily: "monospace"}});
//*********************************************************************************************************
//************************************   Check boxes onChange Forest Mask  ********************************
//*********************************************************************************************************
var GeneralMaskCheckbox = ui.Checkbox({label: 'Choose Forest Mask', value: false, style:{shown: true,
  backgroundColor : "#424457",fontFamily: "monospace",fontWeight: 'bold', fontSize: '18px',color: "#43A5BE"}});
  //
...
///*********************************************************************************************************
var GeneralModelCheckbox = ui.Checkbox({label: 'Choose Machine learning (ML) algorithm', value: false, 
...
// Download maps
 ...
 textAlign: "center", whiteSpace: "nowrap",shown: false }});  
// ********************************************************************************************************
var Label_RemoteSensing    =  ui.Label({value: "> Set period for Sentinel 1/2 collection", 
                            style:{backgroundColor : "#424457", shown: true, fontFamily: "monospace", 
                              fontWeight: 'bold', fontSize: '18px',color: "#43A5BE"   }});
...
///*********************************************************************************************************
var GeneralTypePercCheckbox = ui.Checkbox({label: 'Choose GEDI Rh metric', value: false, style:{shown: true,
  backgroundColor : "#424457", fontFamily: "monospace",fontWeight: 'bold', fontSize: '18px',color: "#43A5BE"}});
//  
...
var documentationLabel = ui.Label({value: "Link to Github CH-GEE web app", style:{
  ...
  targetUrl: "https://github.com/Cesarito2021/Tree Height Mapper GEE"});
